...
iterations = 4          # number of iterating the trial and error
rrr = 7                # should be odd, narrowness of search window for initial values, wideness of beta search scope
...
def fit(ct, rrr, ac, beta, p0, ax):
    ...
    ax[0].plot(np.arange(beginYear, endYear+1, 1), incidence[0]*scale, linewidth=2, label="Experimental Incidence rate")
    ...
    ax[1].plot(np.arange(beginYear, endYear+1, 1), prevalence[1]*scale, linewidth=2, label="Experimental Prevalence rate")
...
with open("Files\Philippines incidence rates.csv", "r") as csvfile:
    csvreader = csv.reader(csvfile, delimiter = ",")
    j = 0
    for row in csvreader:
        if j<3:
...
fit(iterations, rrr, ac, beta, p0, ax)
